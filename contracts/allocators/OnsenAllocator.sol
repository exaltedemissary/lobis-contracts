...
contract OnsenAllocator is Ownable {
  ...
  address immutable treasury; // Lobis Treasury
  ...
  function pendingSushi() external view returns (uint256) {
    ...
    for (uint256 i = 0; i < pids.length; i++) {
      ...
      if (pid != 0) {
        pending = pending.add(
          IMasterChef(masterChef).pendingSushi(pid, address(this))
        );
      }
    }
    return pending;
  ...
  function withdraw(
    ...
    // send LP to treasury
    IERC20(LP).safeTransfer(treasury, _amount);
    // update reserve for this LP
    ITreasury(treasury).updateReserve(LP, _amount);
   ...
   * @param _amount uint
   */
  function enterSushiBarFromTreasury(uint256 _amount) external onlyPolicy {
   ...
   * @param _amount uint
   */
  function exitSushiBar(uint256 _amount) external onlyPolicy {
   ...
   *  @param _pid uint
   */
  function addPool(address _pool, uint256 _pid) external onlyPolicy {
   ...
   *  @param _index uint
   */
  function removePool(address _pool, uint256 _index) external onlyPolicy {
   ...
   *  @param _pid uint
   */
  function emergencyWithdraw(uint256 _pid) external onlyPolicy {
    address LP = pools[_pid];
    ...
    // send LP to treasury
    IERC20(LP).safeTransfer(treasury, balance);
    // update reserve for this LP
    ITreasury(treasury).updateReserve(LP, balance);
  }
}
